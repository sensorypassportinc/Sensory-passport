const CACHE = "sensory-passport-v2";
const APP_SHELL = ["/", "/offline.html", "/manifest.webmanifest", "/app-icon.svg", "/icon-192.png", "/icon-512.png"];
...
self.addEventListener("fetch", event => {
  ...
  if (event.request.mode === "navigate") {
    event.respondWith(fetch(event.request).then(response => { const copy=response.clone(); caches.open(CACHE).then(cache=>cache.put("/",copy)); return response; }).catch(()=>caches.match("/").then(response=>response||caches.match("/offline.html"))));
    return;
  }
